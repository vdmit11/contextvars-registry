...
def test__deferred_default__cannot_be_used__if_existing_context_var_has_default_value():
    ...
    timezone_var_ext = ContextVarDescriptor.from_existing_var(
        timezone_var, deferred_default=lambda: "UTC"
    )
    ...
    timezone_var = ContextVar("timezone_var", default="UTC")
    with pytest.raises(AssertionError):
        ContextVarDescriptor.from_existing_var(timezone_var, deferred_default=lambda: "GMT")
